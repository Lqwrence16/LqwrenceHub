...
-- Changes the current theme and applies it to all registered instances.
...
-- Binds a property to a named theme color so that changing the theme updates it automatically.
...
-- Returns the theme palette of the active theme.
...
-- Plays a tween on an instance.
local function PlayTween(instance, tweenInfo, targetProperties)
    local tween = TweenService:Create(instance, tweenInfo, targetProperties)
    tween:Play()
    return tween
end

-- Creates a new Roblox instance and applies the supplied properties.
...
-- Creates a rounded corner for an object.
...
-- Creates padding inside a frame.
...
-- Creates a border stroke using the active theme colors.
...
-- Creates a list layout for stacking UI elements vertically.
...
-- Safely runs a callback without crashing the entire script if it errors.
...
-- Makes a frame draggable by using the supplied handle.
...
-- Creates a new window object with a title bar, sidebar, and content area.
function WindowSystem.new(config)
    ...
    self.Window = New("Frame", {
        Name = "MainWindow",
        Size = UDim2.fromOffset(Design.WindowSize.X, Design.WindowSize.Y),
...
-- Adds a page button and content holder to the window.
...
-- Creates a page object that can hold cards and controls.
...
function PageSystem:Select()
    for _, page in pairs(self.Window.Pages) do
...
-- Creates a card container with a title area and internal content section.
...
-- Adds a simple label to the card.
...
-- Adds a toggle button to the card.
...
-- Adds a slider UI element to the card.
...
-- Adds a dropdown button to the card.
...
-- Shows a small notification panel on the screen.
...
local function BuildDemoWindow()
    local window = Library:CreateWindow({ Title = "LqwrenceHub" })

    local homePage = window:AddPage("Home")
    local homeCard = homePage:AddCard("Welcome")
    homeCard:AddLabel("This file was rewritten around the theme system from the other script.")
    homeCard:AddButton("Show notification", function()
        Library:Notify("Success", "Your theme-driven UI is running.")
    end)
    homeCard:AddToggle("Enable visuals", false, function(enabled)
        print("Visuals enabled:", enabled)
    end)
    homeCard:AddSlider("Speed", 0, 100, 50, function(value)
        print("Slider value:", value)
    end)
    homeCard:AddInput("Name", "Type here", function(value)
        print("Input value:", value)
    end)
    homeCard:AddDropdown("Mode", { "Auto", "Manual" }, function(value)
        print("Dropdown value:", value)
    end)

    local toolsPage = window:AddPage("Tools")
    local toolsCard = toolsPage:AddCard("Theme")
    toolsCard:AddLabel("You can switch themes by clicking the button below.")
    toolsCard:AddButton("Switch theme", function()
