...
local Tab1 = Window:AddTab("Method 1", "layers", "Tab-level tabboxes")

-- Groupbox on the left
local Group1 = Tab1:AddLeftGroupbox("Regular Group", "settings")
Group1:AddToggle("Toggle1", {
	Text = "Some Toggle",
	Default = false,
})

-- Tabbox ALSO on the left (below the groupbox)
local ColumnTabbox = Tab1:AddLeftTabbox("Settings")

local colTabA = ColumnTabbox:AddTab("General")
colTabA:AddToggle("ColToggle", {
	Text = "Enable Feature",
	Default = true,
	Callback = function(v)
		print("Feature:", v)
...
local colTabB = ColumnTabbox:AddTab("Advanced")
colTabB:AddInput("ColInput", {
	Text = "Config Name",
	Placeholder = "Enter name...",
})

-- Another groupbox on the right
local Group2 = Tab1:AddRightGroupbox("Right Side", "shield")
Group2:AddLabel("This is on the right column")

-- ═══════════════════════════════════════════════════════════
-- METHOD 2: Nested Tabbox inside Groupbox (AddNestedTabbox)
-- Tabbox lives INSIDE a collapsible groupbox
-- Use when you want the tabbox inside a groupbox header
-- ═══════════════════════════════════════════════════════════
local Tab2 = Window:AddTab("Method 2", "box", "Nested inside groupbox")

-- Groupbox that contains a nested tabbox inside it
local NestedGroup = Tab2:AddLeftGroupbox("Nested Tabs", "layers")

-- THIS IS THE NEW FUNCTION — tabbox inside the groupbox
local NestedTabbox = NestedGroup:AddNestedTabbox({ Name = "Settings" })
-- or: NestedGroup:AddNestedTabbox("Settings")

local nestTabA = NestedTabbox:AddTab("General")
nestTabA:AddToggle("NestToggle1", {
...
nestTabA:AddSlider("NestSlider1", {
	Text = "Intensity",
	Default = 50,
	Min = 0,
	Max = 100,
...
local nestTabB = NestedTabbox:AddTab("Advanced")
nestTabB:AddToggle("NestToggle2", {
...
-- You can still add normal elements to the same groupbox
-- (they appear below the nested tabbox)
NestedGroup:AddDivider()
NestedGroup:AddLabel("Elements below the nested tabbox")
NestedGroup:AddButton("Click Me", function()
	print("Clicked!")
end)

-- Right side — regular groupbox for comparison
local RegularGroup = Tab2:AddRightGroupbox("Regular Group", "file-code")
RegularGroup:AddToggle("RegularToggle", { Text = "Normal Toggle" })
RegularGroup:AddSlider("RegularSlider", { Text = "Normal Slider", Default = 25, Min = 0, Max = 100 })

-- ═══════════════════════════════════════════════════════════
-- METHOD 3: Right-side Tabbox (AddRightTabbox)
-- Same as Method 1 but on the right column
-- ═══════════════════════════════════════════════════════════
local Tab3 = Window:AddTab("Method 3", "layout", "Right column tabboxes")

-- Left side — just a simple groupbox
local LeftGroup = Tab3:AddLeftGroupbox("Left Content", "align-left")
LeftGroup:AddToggle("LeftToggle", { Text = "Left Side Toggle" })
LeftGroup:AddButton("Left Button", function()
	print("Left!")
end)

-- Right side — tabbox in the right column
local RightTabbox = Tab3:AddRightTabbox("Options")

local rightTabA = RightTabbox:AddTab("Visual")
rightTabA:AddToggle("ESP", { Text = "Enable ESP", Default = false })
rightTabA:AddToggle("Tracers", { Text = "Show Tracers" })
rightTabA:AddColorPicker("ESPCol", { Default = Color3.new(1, 0, 0), Title = "ESP Color" })

local rightTabB = RightTabbox:AddTab("Combat")
rightTabB:AddToggle("Aimbot", { Text = "Aimbot", Default = false })
rightTabB:AddSlider("AimFov", { Text = "FOV", Default = 100, Min = 10, Max = 500 })
rightTabB:AddDropdown("AimPart", {
	Text = "Target Part",
	Values = { "Head", "Torso", "HumanoidRootPart" },
	Default = "Head",
})

local rightTabC = RightTabbox:AddTab("Misc")
rightTabC:AddToggle("AutoFarm", { Text = "Auto Farm" })
rightTabC:AddInput("FarmKey", { Text = "Farm Keybind", Placeholder = "e.g. E" })

-- ═══════════════════════════════════════════════════════════
-- BONUS: Multiple nested tabboxes in one groupbox
-- ═══════════════════════════════════════════════════════════
local Tab4 = Window:AddTab("Bonus", "star", "Multiple nested tabboxes")

local BonusGroup = Tab4:AddLeftGroupbox("Multi-Nested", "git-branch")

-- First nested tabbox
local Nested1 = BonusGroup:AddNestedTabbox("Category A")
local n1a = Nested1:AddTab("Tab 1")
n1a:AddToggle("T1", { Text = "Option 1" })
n1a:AddToggle("T2", { Text = "Option 2" })
local n1b = Nested1:AddTab("Tab 2")
n1b:AddSlider("S1", { Text = "Value", Default = 10, Min = 0, Max = 50 })

BonusGroup:AddDivider()

-- Second nested tabbox in same groupbox
local Nested2 = BonusGroup:AddNestedTabbox("Category B")
local n2a = Nested2:AddTab("Settings")
n2a:AddDropdown("D1", { Text = "Mode", Values = { "Safe", "Normal", "Aggressive" }, Default = "Normal" })
n2a:AddToggle("T3", { Text = "Auto-Save" })
local n2b = Nested2:AddTab("Debug")
n2b:AddLabel("Debug Info")
n2b:AddButton("Print Debug", function()
